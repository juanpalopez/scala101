...
object Huffman {
  ...
  def weight(tree: CodeTree): Int = tree match {
    case Fork(l,r,cs,fw) => fw
    case Leaf(c,lw) => lw
  }// tree match ...

  def chars(tree: CodeTree): List[Char] = tree match {
    case Fork(l,r,cs,fw) => cs
    case Leaf(c,lw) => List(c)
  }// tree match ...
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      chars.groupBy((c:Char) => c).mapValues(_.size).toList
    }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match{
      case t::Nil => trees
      case t::ts => makeCodeTree(t, ts.head) :: combine(ts.tail)
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def loop(treeI:CodeTree, c:Char, acc:List[Bit]): List[Bit] = treeI match {
        case Fork(l,r, cs, lw) => if (chars(l).contains(c)) loop(l,c,acc :+ 0) else loop(r,c, acc :+ 1)
        case Leaf (c,lw)=> acc
      }
      println(text)
      List(0,1)
      //loop(tree, text.head,List[Bit]()) ::: encode(tree)(text.tail)
    }
