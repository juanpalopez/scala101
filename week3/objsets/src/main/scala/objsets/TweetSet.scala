...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("Empty Set")//new Tweet(null,null,0)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet: TweetList =
    if ((left union right).isEmpty) Nil
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(tw => google.exists(tw.text.contains))
  lazy val appleTweets: TweetSet = allTweets.filter(tw => apple.exists(tw.text.contains))
